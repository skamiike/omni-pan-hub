...
exports.InvalidStoreValueError = void 0;
exports.read = read;
exports.write = write;
...
async function write(store, options) {
    const { address, commitment } = options;
    const config = tempo_1.MultisigConfig.from(options.config);
    assertKey({ address, commitment, config });
    const value = serialize(config);
    await store.setItem(key({ address, commitment }), value);
    if (commitment.toLowerCase() === zeroCommitment)
        await store.setItem(key({ address, commitment: tempo_1.MultisigConfig.getCommitment(config) }), value);
}
function assertKey(options) {
    ...
    if (config.version === 0n) {
        if (!Address.isEqual(tempo_1.MultisigConfig.getAddress(config, { factory: Addresses_js_1.nativeMultisigFactory }), address))
            throw new InvalidStoreValueError();
        if (commitment.toLowerCase() === zeroCommitment)
            return;
    }
    if (tempo_1.MultisigConfig.getCommitment(config).toLowerCase() !==
...
function deserialize(value) {
    try {
        if (value.length > maxStoredValueLength)
            throw new InvalidStoreValueError();
        return tempo_1.MultisigConfig.fromRpc(Json.parse(value));
    }
    catch (cause) {
...
function serialize(config) {
    try {
        const value = Json.stringify(tempo_1.MultisigConfig.toRpc(config));
        if (value.length > maxStoredValueLength)
            throw new InvalidStoreValueError();
        return value;
    }
