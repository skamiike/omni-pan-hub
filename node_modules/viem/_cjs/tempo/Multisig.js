...
function assertConfig(options) {
    ...
    if (config.version === 0n &&
        tempo_1.MultisigConfig.getAddress(config, {
    ...
    if (config.version === 0n && Hex.toBigInt(commitment) === 0n)
        return;
    if (commitment.toLowerCase() !==
        tempo_1.MultisigConfig.getCommitment(config).toLowerCase())
        throw new RpcResponse.InvalidParamsError({
...
async function selectApprovals(options) {
    try {
        return await tempo_1.MultisigOperation.selectApprovals(options);
    }
    catch (error) {
...
async function toTransaction(client, operation) {
    ...
    const current = operation.status === 'pending'
        ...
        : operation;
    const serialized = tempo_1.MultisigOperation.serializeTransaction(current, {
        approvals: approvals.selectedApprovals,
    });
