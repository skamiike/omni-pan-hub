...
exports.getKeyAuthorizationSignPayload = getKeyAuthorizationSignPayload;
...
function fromMultisig(value) {
    ...
    const address = Address.checksum((() => {
        if (typeof value === 'string')
            return value;
        if (value.address === undefined || value.address === 'infer')
            return tempo_1.MultisigConfig.getAddress(config, {
    ...
    const ownerAccounts = (() => {
        ...
        return configInput.owners.flatMap((value) => {
            ...
            return typeof owner === 'string' ? [] : [(0, parseAccount_js_1.parseAccount)(owner)];
        });
    })();
    if (ownerAccounts.some((owner) => owner.source === 'multisig' || isAccessKeyAccount(owner)))
    ...
    const account = {
        ...
        type: 'local',
        async sign({ hash }) {
            return tempo_1.SignatureEnvelope.serialize(await signMultisig(account, { payload: hash }));
        },
        async signMessage() {
        ...
        async signTransaction(transaction, options) {
            ...
            if (request.owner) {
                const owner = (0, parseAccount_js_1.parseAccount)(request.owner);
                if (owner.type !== 'local')
            ...
            if (request.from && !Address.isEqual(request.from, address))
                throw new Error('Multisig account does not match the transaction sender.');
            const signature = await signMultisig(account, {
                config: simulation?.config,
                payload,
                signatures: request.signatures?.map((signature) => (0, Signature_js_1.parseApproval)(signature)),
            });
...
async function signMultisig(account, parameters) {
    ...
    for (const signature of providedSignatures) {
        const address = tempo_1.SignatureEnvelope.extractAddress({
            payload: digest,
            signature,
        }).toLowerCase();
    ...
    for (const owner of owners) {
        ...
        signatures.push((0, Signature_js_1.parseApproval)(await ownerAccount.sign({ hash: digest })));
        signedOwners.add(address);
        weight += Number(owner.weight);
        if (weight >= Number(currentConfig.threshold))
            break;
    ...
    return tempo_1.SignatureEnvelope.from({
        account: account.address,
        config: currentConfig,
        signatures: tempo_1.SignatureEnvelope.sortMultisigApprovals({
            account: account.address,
            config: currentConfig,
            payload,
            signatures,
...
function getKeyAuthorizationSignPayload(account, parameters) {
    ...
    return tempo_1.KeyAuthorization.getSignPayload({
...
async function signKeyAuthorization(account, parameters) {
    ...
    const signature = await (async () => {
        ...
        if (isMultisig) {
            ...
            return tempo_1.SignatureEnvelope.serialize(await signMultisig(account, {
                config: multisigState.config,
                payload: hash,
                signatures: signatures?.map((signature) => (0, Signature_js_1.parseApproval)(signature)),
            }));
        }
        return account.sign({ hash });
    })();
...
function fromBase(parameters) {
    ...
    return {
        ...
        async signTransaction(transaction, options) {
            ...
            const payload = (0, keccak256_js_1.keccak256)(await serializer(presign));
            const { multisigSimulation, from } = transaction;
