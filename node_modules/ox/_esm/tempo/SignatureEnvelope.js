...
function assertKeychainVersion(keychain) {
    if (keychain.version !== 'v1')
        return;
    let inner = keychain.inner;
    while (getType(inner) === 'keychain')
        inner = inner.inner;
...
function assertMultisig(envelope) {
    const missing = [];
    if (!envelope.account)
    ...
    if ((envelope.config.version ?? 0n) === 0n &&
        envelope.config.owners.some((owner) => Address.isEqual(owner.owner, envelope.account)))
        throw new InvalidMultisigApprovalError({
...
export function toRpc(envelope) {
    ...
    if (type === 'multisig')
        return Hex.slice(serialize(envelope), 1);
    throw new CoercionError({ envelope });
}
/**
